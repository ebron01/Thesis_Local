...
class MultiModalGenerator(CaptionModel):
    ...
    def init_weights(self):
        ...
        if self.use_aux:
            self.aux_embeded.weight.data.uniform_(-initrange, initrange)
    ...
    def attention_encoder(self, feats, state, mode, mask):
        if mode == "video":
        ...
        result, weight = attention(self.get_hidden_state(state).squeeze(1), result, mask)
        ...
        for n in range(sent_num):
            ...
            for i in range(seq.size(2)-1):
                ...
                if i >= 1 and seq[:,:,i].sum() == 0:
                    break
                if self.use_video:
                    video, _ = self.attention_encoder(fc_feats[:, n], state, 'video', mask)
                if self.use_img:
                    image, _ = self.attention_encoder(img_feats[:, n], state, 'img', mask)
                if self.use_box:
                    box, _ = self.attention_encoder(box_feats[:, n], state, 'box', mask)
                if self.use_aux:
                    aux_c = aux_c.view(batch_size, self.aux_word_size, -1)
                    aux, attention_weights = self.attention_encoder(aux_c, state, 'aux', mask)
    ...
    def sample_sequential(self, fc_feats, img_feats, box_feats, activity_labels, aux_labels, context=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            if t == 0 : # input <bos>
                it = fc_feats.new_zeros(batch_size, dtype=torch.long)
            if self.use_video:
                video, _ = self.attention_encoder(fc_feats, state, 'video', mask)
            if self.use_img:
                image, _ = self.attention_encoder(img_feats, state, 'img', mask)
            if self.use_box:
                box, _ = self.attention_encoder(box_feats, state, 'box', mask)
            if self.use_aux:
                aux_c = aux_c.view(batch_size, self.aux_word_size, -1)
                aux, attention_weights = self.attention_encoder(aux_c, state, 'aux', mask)
