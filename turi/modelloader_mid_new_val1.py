...
"""
use virtual env turi for experiments
17 may 20'  
"""

# reference_data = tc.load_sframe('/data/conceptualcaptions_0_2.sframe') #conceptualcaptions_0_2.sframe
# loaded_model = tc.load_model('/data/reference_data_all.model')
# # act_data = tc.image_analysis.load_images('./midframeextractor/mid/')
# act_data = tc.image_analysis.load_images('/data/mid_new_val_1/')
# #!!!!don't forget to define k
k = 10
# query = loaded_model.query(act_data[:], k=10) #query = loaded_model.query(act_data[:], k=30)
# data = {}
# key_data = {}
# count = 0
...
# print(len(data))
json_name = 'query_mid_closest%d_new_val1.json' % k
...
for key in split.keys():
    filename = '/data/shared/ActivityNet/activitynet_annotations/ActivityNet_%s.json' % key
...
file = '/data/shared/ActivityNet/conceptualcaptions/Train_GCC-training.tsv'
f = open(file, 'r')
...
for act_key in sorted(best_diff.keys()):
    ...
    split = 'val_1'  # actnet[video_id]['split']
...
json_name = '%dclosest_updated_query_mid_new_val1.json' % k
with open(json_name, 'w') as f:
    json.dump(upd_act, f)
