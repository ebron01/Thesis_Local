...
with open('query_all.json', 'r') as f:
...
for act_key in sorted(best_diff.keys()):
    ...
    for concat_key in sorted(best_diff[act_key].items(), key=lambda kv: kv[1]):
        ...
        if inside_count == 5:
            ...
            tf.text = "Conceptual Captions"
            count = 1
        elif inside_count == 10:
            break
        # this part adds concap pictures
        left = Inches(5)
        top = Inches(1 + (count - 1)*1.25)
        height = Inches(1)
        width = Inches(0.75)
        ...
        p = cf.add_paragraph()
        p.text = concat_key[0] + '.jpg  ' + 'Distance:' + str(best_diff[act_key][concat_key[0]]) + '\n' + str(concat_sentence)
        p.font.size = Pt(10)
        count += 1
        inside_count += 1


prs.save('server_imgsim_best_5_npic.pptx')
