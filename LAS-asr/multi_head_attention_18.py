...
def split_heads(q, k, num_heads, size_per_head):
    ...
    def split_last_dimension_then_transpose(tensor, num_heads, size_per_head):
        ...
        shape0 = tf.shape(tensor)[0]
        tensor = tf.reshape(tensor, [shape0, -1] + [num_heads, size_per_head])
...
class AttentionWrapperState(
      collections.namedtuple("AttentionWrapperState",
                              ...
                              "attention_state", "attention_state_bias"))):
    ...
    def state_size(self):
        ...
        return AttentionWrapperState(
            ...
            attention_state=self._item_or_tuple(
                (a.state_size for a in self._attention_mechanisms), False),
            alignment_history=self._item_or_tuple(
                (a.alignments_size if self._alignment_history else ()
                for a in self._attention_mechanisms), False), # sometimes a TensorArray
            ...
            attention_state_bias=self._item_or_tuple(
                (a.state_size for a in self._attention_mechanisms_bias), True),
            alignment_history_bias=self._item_or_tuple(
                (a.alignments_size if self._alignment_history else ()
                for a in self._attention_mechanisms_bias), True)) # sometimes a TensorArray
        ...
        **NOTE** Please see the initializer documentation for details of how
        ...
        with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            ...
            initial_alignments = [
                ...
                for attention_mechanism in self._attention_mechanisms]
            initial_alignments_bias = [
                attention_mechanism.initial_alignments(batch_size, dtype)
                for attention_mechanism in self._attention_mechanisms_bias]
            return AttentionWrapperState(
                cell_state=cell_state,
                time=array_ops.zeros([], dtype=dtypes.int32),
                attention=_zero_state_tensors(self._attention_layer_size, batch_size,
                                              dtype),
                alignments=self._item_or_tuple(initial_alignments, False),
                attention_state=self._item_or_tuple(
                ...
                alignment_history=self._item_or_tuple(
                    ...
                    for alignment in initial_alignments), False),
                bias_state=cell_state,
                attention_bias=_zero_state_tensors(self._attention_layer_size_bias, batch_size,
                                              dtype),
                alignments_bias=self._item_or_tuple(initial_alignments_bias, True),
                attention_state_bias=self._item_or_tuple(
                    (attention_mechanism.initial_state(batch_size, dtype)
                ...
                alignment_history_bias=self._item_or_tuple(
                    (tensor_array_ops.TensorArray(
                        ...
                        element_shape=alignment.shape)
                    if self._alignment_history else ()
                    for alignment in initial_alignments_bias), True))
